...
```{r}
#Examining the ADOS for TD children, we see that the majority (66.7%) of the TD children have an ADOS score of zero (mean = 0.8, SD = 1.5) comparetively only one (2,9%) of the children with ASD have an ADOS score of zero. Therefore adding TD children to the model might negatively affect predictive power, hereby introducing a systematic  bias in which in overpredicts the ADOS score for TD children while underpredicting the ADOS score for children with ASD. This effect is seen in figure x and x2 where TD seems introduce the expected bias. This bias seems to be diminished if TD is added to the model, as seen in figure x3. Even adding an interaction term between the (as seen in figure x4) seem to still contain the bias, altough it is highly improved. This is caused by the fact that the variation in ADOS for the two groups is highly different.
summary(v1ASD$ADOS)
summary(v1$ADOS[v1$ASD == 0])
sum(v1$ADOS[v1$ASD == 0] == 0)/length(v1$ADOS[v1$ASD == 0])
sum(v1$ADOS[v1$ASD == 1] == 0)/length(v1$ADOS[v1$ASD == 1])
sd(v1$ADOS[v1$ASD == 0])
```
